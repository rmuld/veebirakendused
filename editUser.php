    ...
    if($_SERVER["REQUEST_METHOD"] === "POST"){
        if(isset($_POST["user_data_submit"])){
            ...
            if(isset($_POST["gender_input"]) and !empty($_POST["gender_input"])){
                ...
                $gender_error = "Palun mägi oma sugu!";
            }
            ...
            if(empty($firstname_error) and empty($surname_error) and empty($birth_month_error) and empty($birth_year_error) and empty($birth_day_error) and empty($birth_date_error) and empty($gender_error) and empty($email_error) and empty($password_error) and empty($confirm_password_error)){
                $notice = edit_user($firstname, $surname, $gender, $birth_date, $email, $_POST["password_input"]);
            }
...
?>
    <main>
        ...
        <form method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
            ...
            <input type="email" name="email_input" id="email_input" value="<?php echo $email; ?>" disabled><span><?php echo $email_error; ?></span><br>
            <label for="password_input">Salasõna (min 8 tähemärki):</label><br>
            <input name="password_input" id="password_input" value="<?php echo $password; ?>" type="password"><span><?php echo $password_error; ?></span><br>
            <label for="confirm_password_input">Korrake salasõna:</label><br>
            <input name="confirm_password_input" id="confirm_password_input" value="<?php echo $password; ?>" type="password"><span><?php echo $confirm_password_error; ?></span><br>
            <input name="user_data_submit" type="submit" value="Muuda kasutaja andmed"><span><?php echo $notice; ?></span>
        </form>
